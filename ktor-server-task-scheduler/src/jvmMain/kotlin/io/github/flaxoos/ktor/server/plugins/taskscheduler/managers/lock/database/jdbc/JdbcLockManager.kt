package io.github.flaxoos.ktor.server.plugins.taskscheduler.managers.lock.database.jdbc
...
import io.github.flaxoos.ktor.server.plugins.taskscheduler.managers.lock.database.jdbc.TaskLockTable.concurrencyIndex
import io.github.flaxoos.ktor.server.plugins.taskscheduler.managers.lock.database.jdbc.TaskLockTable.lockedAt
import io.github.flaxoos.ktor.server.plugins.taskscheduler.managers.lock.database.jdbc.TaskLockTable.name
...
internal class JdbcLockManager(
    ...
    override suspend fun acquireLock(task: Task, executionTime: DateTime, concurrencyIndex: Int): JdbcTaskLockKey? {
        ...
        val toLockUntil = taskExecutionInstant.plus(lockExpirationMs, MILLISECOND)
        val updateResult = newSuspendedTransaction(
            application.coroutineContext,
            db = database,
            transactionIsolation = Connection.TRANSACTION_READ_COMMITTED
        ) {
        ...
        return if (updateResult == 1) {
            val lockedUntil = fromUnixMillis(toLockUntil.toEpochMilliseconds())
            logger.debug {
                "${application.host()}: ${executionTime.format2()}: Acquired lock for ${task.name} - $concurrencyIndex, locked until ${lockedUntil.format2()}"
            }
            JdbcTaskLockKey(
                name = task.name,
